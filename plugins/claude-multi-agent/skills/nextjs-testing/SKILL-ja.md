---
name: nextjs-testing-ja
description: Vitest、React Testing Library、Playwrightを使用したNext.jsとReactのテストパターン。コンポーネントテスト、フックテスト、APIルートテスト、Server Actions、統合テスト、E2EテストをTDD方法論に従って網羅します。
user-invocable: false
---

# Next.js テストパターン

Vitest、React Testing Library、Playwrightを使用し、TDD方法論に従ったNext.jsアプリケーションの包括的なテスト戦略です。

## 有効化するタイミング

- 新しいNext.jsのページ、コンポーネント、APIルートを作成するとき
- React Server ComponentsやClient Componentsをテストするとき
- Next.jsプロジェクトのテスト基盤を構築するとき
- カスタムReactフックをテストするとき
- Next.jsの内部機能（router、navigation、Image、Link）をモックするとき
- PlaywrightでユーザーフローのE2Eテストを書くとき
- Next.jsコードベースのテストカバレッジを見直すとき
...
```
RED      → 期待する振る舞いを記述した失敗するテストを書く
GREEN    → テストをパスさせる最小限の実装を書く
REFACTOR → テストをグリーンに保ちながらコードとテストを整理する
REPEAT   → 次の要件に進む
```
...
```
         /\
        /E2E\          <- 少数、低速、高い信頼性（Playwright）
       /------\
      / Integr \       <- 中程度、ページレベルのフロー
     /----------\
    /  Unit Tests \    <- 多数、高速、独立（Vitest + RTL）
   /--------------\
```

ユニットレベルで最も多くのテストを実行します。統合テストはページレベルの
合成を検証します。E2Eテストは重要なユーザージャーニーをエンドツーエンドでカバーします。
...
VitestはNext.jsプロジェクトに推奨されるテストランナーです。Jestより高速で、
ESMをネイティブにサポートし、互換性のあるAPIを持っています。
...
// 各テスト後にコンポーネントを自動アンマウントする
afterEach(() => {
  cleanup()
})

// next/navigation をグローバルにモックする
...
### Vitest vs Jest — Vitestを選ぶ理由

| 比較項目 | Vitest | Jest + next/jest |
|---------|--------|-----------------|
| 速度 | ネイティブESM、高速な起動 | Babelトランスフォームが必要 |
| 設定 | 単一のvite設定 | jest.config.js + babel が別途必要 |
| ESMサポート | ファーストクラス | 回避策が必要 |
| API互換性 | Jest互換 | N/A |
| ウォッチモード | HMRベース、即時反映 | 再実行が遅い |
| SWC/Turbopackとの整合 | 優れている | 劣っている |

### Jest の設定（Jestを使わなければならない場合）
...
ユーザーに見えるアクセシブルなロールやテキストでクエリを行います。
CSSクラス名や内部実装の詳細でのクエリは避けてください。

```typescript
// ✅ GOOD: ロールと表示テキストでクエリする
...
// ❌ BAD: 実装の詳細でクエリする
screen.getByTestId('submit-btn')           // 壊れやすく、ユーザー向けでない
document.querySelector('.submit-button')   // RTLを完全に回避している
screen.getByClassName('btn-primary')       // RTLのAPIではない
```
...
Server Componentsは非同期関数です。コンポーネントをawaitして、
解決されたJSXを`render`に渡してレンダリングします。
...
リアルなユーザーインタラクションのシミュレーションには常に`userEvent`を優先します。
`fireEvent`は合成DOMイベントを直接テストする場合にのみ使用します。

```typescript
// ✅ GOOD: userEvent は実際のブラウザの動作をシミュレートする
const user = userEvent.setup()
...
describe('useFetch', () => {
  ...
  it('does not update state after unmount (no memory leak)', async () => {
    ...
    // アンマウント後に解決する — ステートの更新が発生してはならない
    resolvePromise({ data: 'late' })

    // コンポーネントが消えているのでステートのアサーションはしない。エラーが投げられないことを確認するだけ
    expect(result.current.loading).toBe(true)
  })
})
```
...
Server Actionsは単純な非同期関数です。依存関係をモックしてピュアなユニットとして
テストします。Next.jsのキャッシュ関数（`revalidatePath`、`revalidateTag`）も
モックする必要があります。
...
統合テストはデータフェッチとルーティングのコンテキストを含む完全な
ページをレンダリングします。外部I/O（データベース、fetch）のみをモックします。

```typescript
// app/posts/page.tsx (Server Component ページ)
...
test.describe('Authentication', () => {
  ...
  test('logout clears session and redirects to home', async ({ page, context }) => {
    // storageState を使って認証済みの状態をセットアップする
    await page.goto('/login')
    await page.getByLabel('Email').fill('user@example.com')
    await page.getByLabel('Password').fill('password123')
    await page.getByRole('button', { name: 'Sign In' }).click()
...
// storageState を使ってテスト間で認証済みの状態を再利用する
...
```typescript
// ✅ GOOD: describe/it の外でモジュールレベルにモックする
import { vi } from 'vitest'
...
// ❌ BAD: テストブロック内でモックする — ホイスティングの動作が信頼できない
test('bad pattern', () => {
  vi.mock('@/lib/stripe', () => ({ ... })) // ホイスティングの問題
})
```

### MSW を使った fetch のモック（推奨）

MSWはネットワークレベルでインターセプトするため、コンポーネントコードを
変更せずにリアルなテストが可能です。
...
// ✅ GOOD: 全テスト用に vitest.config.ts で設定する
...
```typescript
// ✅ GOOD: importOriginal を使って必要な部分だけモックする
...
│   ├── app/
│   │   ├── page.tsx
│   │   ├── page.test.tsx        # コロケーションされたユニット/統合テスト
│   │   ├── api/
│   │   │   └── users/
│   │   │       ├── route.ts
│   │   │       └── route.test.ts
...
│   │   ├── Button/
│   │   │   ├── Button.tsx
│   │   │   └── Button.test.tsx  # コンポーネントとコロケーションする
│   │   └── UserCard/
│   │       ├── UserCard.tsx
│   │       └── UserCard.test.tsx
│   └── hooks/
...
│   ├── navigation.spec.ts
│   └── .auth/
│       └── user.json            # Playwright の保存済み認証状態
├── vitest.config.ts
└── playwright.config.ts
```
...
| 種類 | 規則 | 例 |
|------|-----------|---------|
| ユニット / コンポーネントテスト | `<Name>.test.tsx` | `Button.test.tsx` |
| 統合テスト | `<Name>.test.tsx` | `PostsPage.test.tsx` |
| E2Eテスト | `<name>.spec.ts` | `auth.spec.ts` |
| テストユーティリティ | `tests/` 内の `<name>.ts` | `render-helpers.ts` |
| MSWハンドラー | `handlers.ts` | `tests/msw/handlers.ts` |
...
- TDDに従う: 実装の前にテストを書く
- アクセシブルなロール、ラベル、表示テキストでクエリする（`getByRole`、`getByLabelText`、`getByText`）
- リアルなインタラクションのシミュレーションには `fireEvent` より `userEvent` を使う
- 実装の深部ではなくモジュールの境界でモックする
- `fetch` をグローバルにモックする代わりにAPIモッキングにはMSWを使う
- ユニットテストは高速に保つ; Playwrightは重要なユーザージャーニーに限定する
- テストファイルはテスト対象のソースファイルとコロケーションする
- テスト汚染を防ぐために `beforeEach` で `vi.clearAllMocks()` を使う
- 非同期ステートのアサーションには `waitFor` と `findBy*` を使う
- ハッピーパスだけでなく、エラー状態とローディング状態もテストする
- リグレッションを防ぐためにCIでカバレッジの閾値を設定する

**DON'T（やってはいけないこと）:**

- 他に選択肢がない場合を除いて `data-testid` でクエリしない — ユーザー体験を反映しない
- テスト内部で実装関数を直接インポートして呼び出さない
- CSSクラス名やインラインスタイルにアサーションをかけない
- ナビゲーションやフォーム送信をトリガーするボタンのインタラクションに `fireEvent.click` を使わない
- `next/router`（Pages Router）をモックしない — App Routerには `next/navigation` を使う
- Server ComponentsをClient Componentとしてインポートしてテストしない
- 実行順序に依存するテストを書かない
- 理由を文書化せずにテストセットアップで `console.error` の抑制を残さない
- エラー分岐のテストを省略しない — それが最も価値のあるテストである
...
|------|-----------|
| ユニットテストランナー | Vitest |
| コンポーネントのレンダリング | `@testing-library/react` — `render` |
| ロールでのクエリ | `screen.getByRole(role, { name })` |
| ラベルでのクエリ | `screen.getByLabelText(label)` |
| 非同期要素のクエリ | `screen.findByRole(...)` |
| ユーザーインタラクション | `@testing-library/user-event` — `userEvent.setup()` |
| DOM存在のアサーション | `expect(el).toBeInTheDocument()` |
| テキスト内容のアサーション | `expect(el).toHaveTextContent(text)` |
| フォーム値のアサーション | `expect(input).toHaveValue(value)` |
| フォーカスのアサーション | `expect(el).toHaveFocus()` |
| 非同期アサーション | `await waitFor(() => expect(...))` |
| モジュールモック | `vi.mock('module', () => ({ ... }))` |
| 関数スパイ | `vi.spyOn(obj, 'method')` |
| 部分モック | `vi.mock(..., async orig => ({ ...await orig(), fn: vi.fn() }))` |
| 環境変数モック | `vi.stubEnv('KEY', 'value')` |
| フックテスト | `renderHook(() => useMyHook())` |
| コンテキスト付きフック | `renderHook(() => useHook(), { wrapper })` |
| フックのステート変更 | `act(() => result.current.fn())` |
| API/fetch モック | MSW — `http.get('/path', () => HttpResponse.json(...))` |
| Server Action モック | `vi.mock('next/cache')` + モック関数の依存関係 |
| E2E ナビゲーション | `page.goto(url)` + `expect(page).toHaveURL(url)` |
| E2E フォーム入力 | `page.getByLabel('Name').fill('Alice')` |
| E2E クリック | `page.getByRole('button', { name: 'Submit' }).click()` |
| E2E アサーション | `expect(locator).toBeVisible()` / `toHaveText()` |
| カバレッジレポート | `vitest run --coverage` |
| E2E レポート | `playwright show-report` |

**Remember（覚えておくこと）**: テストは実行可能な仕様書です。実装の詳細ではなく、ユーザーの視点から振る舞いを記述するように書きましょう。
